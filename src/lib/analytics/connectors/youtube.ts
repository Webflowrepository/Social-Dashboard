...
export async function syncYouTube(env: Env, range: DateRange): Promise<{ metrics: NormalizedMetric[]; content: ContentPerformance[] }> {
  ...
  let ids = [...byVideo.keys()].filter(Boolean);
  if (!analyticsAvailable) {
    ...
    ids = (search.items || []).map((item: { id?: { videoId?: string } }) => item.id?.videoId || "").filter(Boolean);
  }
